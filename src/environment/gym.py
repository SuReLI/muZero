...
class GymWrapper:
    def __init__(
        self, env_id
    ):  # takes as input the name of the desired environment (for example "CartPole-v1")
    ...
    def mask(self) -> torch.Tensor:
        ...
        )  # allow all actions for a simple environment
            ...
            print(
                " ".join(
                    (
                        "X"
                        if cell == 1
                        else " " if cell == 0 else "E" if cell == "E" else "A"
                    )
                    for cell in row
                )
            )
