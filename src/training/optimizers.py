...
class Optimizers:
    ...
    def __init__(
        ...
        self.lr = lr
        self.opt_h = (
            opt_h if opt_h is not None else optim.Adam(h.parameters(), lr=self.lr)
        )
        self.opt_g = (
            opt_g if opt_g is not None else optim.Adam(g.parameters(), lr=self.lr)
        )
        self.opt_f = (
            opt_f if opt_f is not None else optim.Adam(f.parameters(), lr=self.lr)
        )
    ...
    def basic_optimizer(cls, h: nn.Module, g: nn.Module, f: nn.Module) -> "Optimizers":
        ...
        return cls(
            ...
            g,
            f,
            optim.Adam(h.parameters(), lr=lr),
            optim.Adam(g.parameters(), lr=lr),
            optim.Adam(f.parameters(), lr=lr),
        )
